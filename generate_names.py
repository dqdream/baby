...
class BabyName():
    def __init__(self, config={}, name_dict={}, is_score=True, use_proxy=False, is_check_component=False,
    ...
    def check_duplicate_names(self, name):
        ...
        #     boys_total = "男生0.00%"
        #     name_sex = "未知"
        url = "https://www.qmz99.com/i/?type=chongming&name=" + name
        params = {}
        content = requests.post(url=url, data=params, headers=self.headers, timeout=5, proxies=proxies).content
        soup = BeautifulSoup(content, 'html.parser')
        node_cont = '暂无数据'
        yuyi = ''
        for node in soup.find_all("span", class_='count'):
            node_cont = node['data-num']

        for node in soup.find_all("div", class_='chongming-yuyi'):
            for child in node.contents:
                yuyi = yuyi + child.text

        print('寓意：'+yuyi)
        names_total = node_cont
        girls_total = "女生0人"
        boys_total = "男生0人"
        name_sex = "男"
        return names_total, girls_total, boys_total, name_sex, yuyi
    ...
    def compute_name_score(self, name_postfix):
        ...
        self.fromName(lines_data, name_postfix, 'shijing')
        self.fromName(lines_data, name_postfix, 'cifu')
        self.fromName(lines_data, name_postfix, 'chuci')
        self.fromName(lines_data, name_postfix, 'gushi')
        self.fromName(lines_data, name_postfix, 'shijing')
        self.fromName(lines_data, name_postfix, 'songci')
        self.fromName(lines_data, name_postfix, 'tangshi')
        self.fromName(lines_data, name_postfix, 'yuefu')

        names_total, girls_total, boys_total, name_sex, yuyi = self.check_duplicate_names(full_name)
        ...
        result_data['yuyi'] = yuyi
        result_data['from'] = name_postfix
    ...
    def fromName(self, lines_data, name_postfix, file_name):
        with open(self.NAME_DICT[file_name], 'r') as load_f:
            json_obj = json.load(load_f)
            for item in json_obj:
                json_content = item['content']
                if json_content is not None:
                    ls = json_content.split('。')
                    if ls is not None:
                        for le in ls:
                            a = le.find(name_postfix)
                            if a != -1:
                                lines_data.append(le)
                                print(item['author']+'<<' + item['title'] +">>:" + le)
                                if len(lines_data) > 3:
                                    break
                            else:
                                for i in unicode(name_postfix, 'utf-8'):
                                    if len(lines_data) > 3:
                                        break
                                    a = le.find(i)
                                    if a != -1:
    ...
    def run(self):
        ...
        # 输出文件列名
        _fieldnames = ["ID", "姓名", "拼音", "八字评分", "五格评分", "命主星宿", "命宫", "笔划数", "重名数", "女生占比", "男生占比", "性别偏向", "总分", '寓意']
